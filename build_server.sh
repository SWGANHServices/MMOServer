...
version=0.7.0       # mmoserver project version
...
if [ ! -d $basedir/deps ]; then
    ...
    if [ ! -f $basedir/$filename ]; then
        wget -nc --no-check-certificate https://github.com/SWGANHServices/MMOServer_Legacy/releases/download/downloads/$filename
    fi
